...
public class SplitSearch2 {
    ...
    public boolean label(IntVar[] vars) {
        ...
        if (!consistent) {
            ...
            store.impose(choice.getGreaterThanEqConstraint());
            ...
            if (consistent) {
                ...
                store.impose(choice.getLesserThanConstraint());
    ...
    public class ChoicePoint {
        ...
        public ChoicePoint (IntVar[] v) {
            var = selectMostConstrainedVariable(v);
            minValue = selectMinValue(var);
            maxValue = selectMaxValue(var);
            if((maxValue + minValue) % 2 == 0){
                midValue = (minValue + maxValue)/2;
            } else {
                midValue = (minValue + maxValue + 1)/2;
            }
        }
        ...
        IntVar selectMostConstrainedVariable(IntVar[] v) {
            int largestConstraint = 0;
            int index = 0;

            for(int i = 0; i < v.length - 1; i++) {
                if(v[i].sizeConstraints() > largestConstraint) {
                    largestConstraint = v[i].sizeConstraints();
                    index = i;
                }
            }
         ...
         */
        int selectMinValue(IntVar v) {
            return v.min();
        }

        int selectMaxValue(IntVar v) { return v.max(); }

        public PrimitiveConstraint getGreaterThanEqConstraint(){return new XgteqC(var, midValue); }

        public PrimitiveConstraint getLesserThanConstraint() { return new XltC(var, midValue); }
    }
}
